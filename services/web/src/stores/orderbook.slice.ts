...
const ORDERBOOK_HOST =
  import.meta.env.VITE_APP_ORDERBOOK_URL || process.env.VITE_APP_ORDERBOOK_URL;
...
> = (set) => ({
  connectOrderbook: () => {
    const socket = io(ORDERBOOK_HOST);
    ...
    socket.on("message", (event: string) => {
      ...
      message?.data?.forEach((_data) => {
        ...
        const filteredBids = bids
          ? sumVolumeByPrice(bids, true).slice(0, 15)
          : [];
